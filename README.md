...
there is more. That cap cannot be expressed in CSS, because the gallery sits in a
`1fr` row and a `max-height: 100%` would just make the row grow, so
...
**Netlify Forms**, so there is no backend and no third-party service. Netlify detects it at
...
than the 900 the rest of the headings use, because asking for a weight the family does
...
- `data-project-title`, `data-project-category`, `data-project-description`,
  which populate the nav menu entry.
